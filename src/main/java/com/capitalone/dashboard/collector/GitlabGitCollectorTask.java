...
public class GitlabGitCollectorTask  extends CollectorTask<Collector> {
    ...
    public void collect(Collector collector) {
		...
		for (GitlabGitRepo repo : enabledRepos(collector)) {
			...
			try {
				...
				log(repo.getOptions().toString() + "::" + repo.getBranch() + ":: get commits");
				...
				log(repo.getOptions().toString() + "::" + repo.getBranch() + " get issues");
				...
				log(repo.getOptions().toString() + "::" + repo.getBranch() + "::get pulls");
	...
	private void setupProxy() {
        String proxyUrl = gitlabSettings.getProxyUrl();
        String proxyPort = gitlabSettings.getProxyPort();
        String proxyUser = gitlabSettings.getProxyUser();
        String proxyPassword = gitlabSettings.getProxyPassword();
        ...
        if (!StringUtils.isEmpty(proxyUrl) && !StringUtils.isEmpty(proxyPort)) {
            ...
            if (!StringUtils.isEmpty(proxyUser) && !StringUtils.isEmpty(proxyPassword)) {
				...
				logBanner("proxyUrl: "+proxyUrl);
				logBanner("proxyPort: "+proxyPort);
				logBanner("proxyUser: "+proxyUser);
				logBanner("proxyPassword: "+proxyPassword);
